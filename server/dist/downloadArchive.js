 ...
 */
async function expandItem(graphClient, driveId, itemId, basePath, result) {
    ...
    if (item.folder) {
        await expandFolder(graphClient, driveId, itemId, basePath ? `${basePath}/${itemName}` : itemName, result);
    }
    else {
        result.push({
            itemId,
 ...
 * @returns Promise<void>
 */
async function expandFolder(graphClient, driveId, folderId, folderPath, result) {
    let endpoint = `/drives/${driveId}/items/${folderId}/children`;
    while (endpoint) {
        const page = await graphClient.api(endpoint).select("id,name,folder,file,size").get();
        const children = page.value ?? [];
        for (const child of children) {
            const childId = child.id ?? "";
            const childName = child.name ?? "";
            if (child.folder) {
                await expandFolder(graphClient, driveId, childId, `${folderPath}/${childName}`, result);
            }
            else {
                result.push({
                    itemId: childId,
 ...
 */
async function resolveDownloadUrl(graphClient, graphToken, driveId, itemId) {
    const item = (await graphClient
        .api(`/drives/${driveId}/items/${itemId}`)
        .get());
 ...
 */
async function startDownloadJob(containerId, itemIds, userToken, ownerOid) {
...
function getJobProgress(jobId, requesterOid) {
    ...
    if (requesterOid !== undefined && job.ownerOid !== requesterOid)
        return null;
    const { manifest: _ignored, createdAt: _c, completedAt: _ca, ownerOid: _o, ...progress } = job;
    return progress;
}
/**
 * 读取已完成任务的下载清单。
 ...
 * @returns Promise<void>
 */
async function processJob(jobId, containerId, itemIds, userToken) {
    const job = jobs.get(jobId);
    job.status = "preparing";
    job.currentItem = "Initialising...";
    let graphToken;
    try {
        graphToken = await (0, auth_1.getGraphToken)(userToken);
    }
    catch (err) {
        job.status = "failed";
        job.completedAt = Date.now();
        job.errors.push(`Graph token error: ${err instanceof Error ? err.message : String(err)}`);
        return;
    }
    ...
    for (const itemId of itemIds) {
        try {
            ...
            job.errors.push(`Failed to expand item ${itemId}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    for (let i = 0; i < flatFiles.length; i++) {
        ...
        job.processedFiles = i;
        try {
            const downloadUrl = await resolveDownloadUrl(graphClient, graphToken, containerId, file.itemId);
            manifestItems.push({
                itemId: file.itemId,
                name: file.name,
            ...
            job.errors.push(`Error preparing ${file.relativePath}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
