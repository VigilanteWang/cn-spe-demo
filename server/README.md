...
> Node.js + TypeScript + Restify 实现的 SharePoint Embedded 后端 API

本文档为所有后端服务器文件详细讲解了代码含义和实现原理，适合初级开发者快速理解项目架构。
...
| 模块     | 文件                 | 职责                                     |
| -------- | -------------------- | ---------------------------------------- |
| 主入口   | `index.ts`           | 启动 Restify 服务器、注册路由、配置 CORS |
| 权限验证 | `auth.ts`            | JWT 验证、OBO 流程、Graph 客户端创建     |
...
| 归档下载 | `download/`          | ZIP 下载准备任务（启动/进度/清单端点）   |
| 配置管理 | `config.ts`          | 环境变量读取、云环境选择                 |
| 权限常量 | `common/scopes.ts`   | SPE 权限字符串常量                       |

> \*deleteItems 路由在 index.ts 中定义
...
**文件位置**: `server/config.ts`

读取后端所需的环境变量，支持全球版和世纪互联版两种云环境：

| 环境变量                      | 说明                               |
| ----------------------------- | ---------------------------------- |
| `API_ENTRA_APP_CLIENT_ID`     | 后端 Entra App 的客户端 ID         |
| `API_ENTRA_APP_CLIENT_SECRET` | 后端 Entra App 的客户端密钥        |
| `API_ENTRA_APP_TENANT_ID`     | Azure AD 租户 ID                   |
| `CONTAINER_TYPE_ID`           | SPE 容器类型 ID                    |
| `CLOUD_ENV`                   | 云环境：`global`（默认）或 `china` |
| `PORT`                        | 服务器端口（默认 3001）            |

## download/ - 归档下载

**文件位置**: `server/download/`

提供基于任务的异步下载准备机制：

1. 调用方通过 `startDownloadJob()` 启动任务，获取 jobId
2. 轮询 `getJobProgress()` 查看进度（queued → preparing → ready / failed）
3. 状态为 ready 时，调用 `getJobManifest()` 读取清单
4. 浏览器根据 manifest 逐项流式下载并在前端压缩成 ZIP
