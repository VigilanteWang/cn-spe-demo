 ...
 * 文件列表表格。
 *
 * 这个组件只负责把行点击、预览、打开文件夹和“管理权限”动作抛给外层，
 * 不在表格内部承载 item 权限业务状态。
 *
 * @param props 组件属性。
 * @returns DataGrid UI。
 */
  ...
  const columns = useMemo<TableColumnDefinition<IDriveItemExtended>[]>(
    () => [
      ...
      createTableColumn({
        ...
        renderCell: (driveItem) => (
          // PersonCell 负责头像、姓名、在线状态的组合展示，避免在表格里重复拼装 UI。
          <PersonCell
  ...
  return (
    ...
    */
    <DataGrid
      items={driveItems}
      columns={columns}
      // 使用 Graph 返回的 `driveItem.id` 作为稳定键，避免排序后行状态错位。
