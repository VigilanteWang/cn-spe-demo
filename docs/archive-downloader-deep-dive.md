...
for (const item of ...) {         new Zip((error, data, final) => {
  const response = await            // 压缩好了，data 来了！
    fetch(item.url);                // 但不知道何时结束……
  entry.push(chunk);              });
}
```

两种范式不能**直接**混合。这里更准确的说法是：

- 你当然可以把回调函数写成 `async`，也可以在回调函数里面写 `await`。
- 但如果**调用这个回调的库**没有 `await` 这个回调返回的 Promise，那么这个 `await` 只能让“回调函数自己”暂停，不能让外层流程暂停。
- 这里的 `fflate.Zip` 就属于这种情况：它会同步触发回调，但不会替你等待回调里异步写盘完成。

看一个最小伪代码就更容易理解：

```typescript
function fakeZip(callback: (data: string) => Promise<void> | void) {
  callback("chunk1");
  callback("chunk2");
  console.log("fakeZip 已继续往下执行");
}

fakeZip(async (data) => {
  await slowWrite(data);
  console.log("写完", data);
});
```

上面这段代码里，`await slowWrite(data)` 确实会让回调函数自己暂停；
但如果 `fakeZip` 不主动 `await callback(...)`，那么 `fakeZip` 还是会继续往下执行。

这就是当前实现真正要解决的问题：不是“语法上不能在回调里写 `await`”，而是“不能指望库帮你等待这个 `await`”。
...
1. `writeChain` 只负责把每次 `writable.write(...)` 串成一条有顺序的异步队列。
2. `final` 负责告诉我们：ZIP 已经没有更多输出了，不会再往 `writeChain` 后面追加新任务了。
3. 外层 `new Promise` 负责把“`final` 到来 + 最新 `writeChain` 全部完成”转换成一个可以被 `await` 的总完成信号。

也就是说，`writeChain` 解决的是“**按顺序写**”，`await new Promise(...)` 解决的是“**什么时候整个流程才算真的完成**”。
