...
## 6. 为什么这里还配合了 `useCallback`

你会看到现在很多传给 Hook 或子组件的方法，都会经过 `useCallback`。`useCallback` 的作用：只有依赖项变了，这个函数才会重新创建。React 使用 Object.is 来比较依赖项是否变化，原始类型比较值，引用类型比较引用。
...
- 让函数引用更稳定
- 明确声明“这个函数依赖哪些状态”，只有 `[folderId, loadItems]` 引用或值变化，才会新建
- 方便子 Hook 和子组件进行依赖追踪
...
当某个 Hook 需要调用外部能力时，优先把能力作为参数传进去，而不是把所有逻辑都写在 Hook 里面。
