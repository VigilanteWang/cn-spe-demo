...
**SharePoint Embedded 是一个纯 API 驱动的文件与文档管理平台。** 它把 Microsoft 365 存储、协作、合规能力"嵌入"到你自己的应用中，但 **不暴露任何 SharePoint 站点界面**，即无界面 SharePoint。
...
- **Container Type（容器类型）**：相当于 Container 的蓝图——规定了创建出Container 的访问权限、计费归属和配置。每个 Container Type 与一个 Owning Application 严格 1:1 绑定。
- **Owning Application（拥有应用）**：在 Microsoft Entra ID 中注册的应用，是 Container Type 的创建者和管理者。
- **Guest Application（来宾应用）**：经 Owning Application 授权后，也可以访问该 Container Type 下的 Container （例如用于备份、审计等场景）。
...
一个 Consuming Tenant 中可以部署多个 SPE 应用，每个应用只能访问自己 Container Type 对应的 Container ——** Container 之间天然隔离**。不过也可以通过 Guest Application 机制让多个应用共享同一组容器。

#### 场景示例

> Contoso（ISV）开发了一款 HR App，部署到 Fabrikam 的租户。Fabrikam 同时也自建了一个 Audit App。两个应用各自拥有独立的 Container Type，互不可见。Contoso 是 HR App 的 Owning Tenant，Fabrikam 既是 HR App 的 Consuming Tenant，也是 Audit App 的 Owning + Consuming Tenant。
>
> ![multi-app-architecture](https://learn.microsoft.com/en-us/sharepoint/dev/embedded/images/apparchexample.png)
...
通过 Container Type Registration API（ Container Type 注册 API）配置，决定了应用对某个 Container Type 下所有容器能做什么操作。常用权限如下：
...
> 上面的请求会在 Consuming Tenant 中创建一个新Container，调用者自动成为 Owner。
