...
**SharePoint Embedded 是一个纯 API 驱动的文件与文档管理平台。** 它把 Microsoft 365 存储、协作、合规能力"嵌入"到你自己的 App 中，但 **不暴露任何 SharePoint 站点界面**，即无界面 SharePoint。
...
SPE App 的共享能力默认继承 Consuming Tenant 的全局共享策略。但 Consuming Tenant 管理员可以通过 `Set-SPOApplication` 为某个 SPE App 单独配置不同的外部共享策略——即使全局禁止访客共享，也可以让特定 App 允许。
...
| "用了 SPE 就不需要 Admin Consent"                         | Consuming Tenant 的管理员必须对 App 授予 Admin Consent 后才能使用                                                   |
...
| "纯应用模式（App-only）的权限和代理模式（Delegated）一样" | App-only 获得 Container Type 级别的全部权限，不受用户角色限制；Delegated 的有效权限是 App 权限和用户角色的 **交集** |
...
| Azure AD                  | 微软身份平台（原 Azure AD） | 应用注册与身份认证平台                        |
